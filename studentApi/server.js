...
app.put('/std/:id', function (req, res) {
        ...
        if (!std_id || !std ) {
                return res.status(400).send({ error: user, message: 'Plaese provide student data and student_id' });
        }
...
app.delete('/std/:id', function (req, res) {
        ...
        if (!std_id) {
                return res.status(400).send({ error: user, message: 'Plaese provide std_id' });
        }
        dbConn.query('DELETE FROM student WHERE std_id = ?', std_id, function (error, result, fields ) {
                if (error) throw error;
